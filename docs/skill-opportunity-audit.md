...
**Deferred to Phase 2 (unchanged):** the S1 promotion and `/handoff` generalization gates; the `Write-WipConflictReport` stash-residuals extension (Tools flag); the WebCatalog/Vault DB-migration Vault section (open carry-over). Owner rulings honored throughout: `skipDangerousModePermissionPrompt`, broad `Bash(git:*)`, and pyRevit `bypassPermissions` not re-flagged; GSD stays parked; retired PostProcess repos untouched; wip-conflict and sync-machines remain rejected as skills.

**Phase 2 riders completed later the same day (owner request):**
- **S3 landing (vault `4e022e2`):** `remote-access.md` gained the "Remote execution from agent sessions" section (scp-a-script + `pwsh -NoProfile`, the two-shells-parse-first `$`/subexpression pitfall with the verified failure case, BatchMode/ConnectTimeout, per-user home paths, no double-hop pwsh, both-ends verify + temp cleanup, pinned agent launches); the remote-wip one-liner's quoting footgun fixed in the same pass. Home memory's `reference_workmachine_profile.md` slimmed to a pointer so the Vault page is the single source.
- **N4.3 stash drop:** the root repo's `unwip-autostash` (base `957fc70`, 2026-06-03) verified per the `feedback_stash_pop_conflict.md` procedure — its tracked content was exactly the rule-6 CLAUDE.md line (confirmed present in HEAD via `git grep`), its untracked component only five stale `Recovery/` Revit crash dumps (the designated never-ferry junk; dir no longer on disk) — then dropped. Root stash list is now empty.
